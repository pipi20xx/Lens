...
import { APP_ACTIVITY_SUSPEND_DELAY_MS } from '../host/appActivityLifecycle'
import {
...
export function useGlassOpticalRenderer(options: UseGlassOpticalRendererOptions) {
  ...
  function setupObservers() {
    ...
    observeMutationRoot(document.querySelector('.v-application'), true)
    observeMutationRoot(document.querySelector('.v-overlay-container'), true)
  ...
  async function initializeRenderer(releaseContext = true) {
    ...
    try {
      ...
      // 等待 canvas 挂载到 DOM 并获得非零尺寸（异步组件加载时 canvas ref 可能先于 DOM 赋值）
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        ...
        // canvas 在 DOM 中但尺寸为 0，等一帧让布局完成
        if (canvas.isConnected) {
          canvasReady = true // 尺寸为 0 也可以获取 context，后续 resizeRenderer 会修正
          break
        }
        await nextTick()
      }
      if (!canvasReady || !options.canvas.value) {
        throw new Error('Canvas not ready after retries')
      }
      ...
      if (!context) {
        throw new Error('WebGL2 context unavailable — canvas may not be in DOM or WebGL is disabled')
      }
